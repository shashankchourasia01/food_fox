...
const MiniCart = ({ onClose }) => {
  ...
  if (cartItems.length === 0) {
    return (
      <div ref={cartRef} className="w-80 bg-white rounded-lg shadow-xl border border-gray-200 p-6">
  ...
  return (
    <div ref={cartRef} className="w-80 sm:w-96 bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden">
      {/* Header */}
      ...
      {/* Cart Items - ✅ Fixed: No Math.random() in render */}
      <div className="max-h-96 overflow-y-auto">
        {cartItems.map((item) => {
          // ✅ Use product ID as key (always unique)
          const itemKey = item.product?._id || item.id;
